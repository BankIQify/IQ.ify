  ...
  return (
    <div className="space-y-2">
      ...
      <div className="mt-3 p-3 bg-yellow-50 dark:bg-yellow-950 border border-yellow-100 dark:border-yellow-900 rounded-md">
        <p className="text-sm">
          <span className="font-medium">Important:</span> When configuring in Make or other platforms that expect 
          standard authentication headers, you can now use either the custom <code className="text-xs font-bold">x-webhook-key</code> 
          or the standard <code className="text-xs font-bold">Authorization</code> header with your API key.
        </p>
      </div>
    </div>
  );
